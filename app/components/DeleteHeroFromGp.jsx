...
import { deleteHeroFromGp } from "@/services/Requests";

const DeleteHeroFromGp = ({ gp_id, colNumber, update }) => {
    ...
    const sendData = async (hero_id) => {
        try {
            ...
            const { data } = await deleteHeroFromGp({ gp_id, colNumber, hero_id });
            setRequestStatus({
                error: null,
                loading: false,
                response: null
    ...
    return (
        <>
            ...
            {!requestStatus.error && !requestStatus.response && !requestStatus.loading &&
                <div className='flex p-3 border-2 rounded-lg border-dashed justify-center text-red-500 border-red-500'
...
export default DeleteHeroFromGp;